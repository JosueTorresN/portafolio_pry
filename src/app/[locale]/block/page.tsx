...
export default function BlockPage() {
    ...
    return (
        <main className="min-h-screen bg-bg text-text p-4 sm:p-6 md:p-12">
            <div className="max-w-4xl mx-auto">
                {/* Header */}
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h2 className="text-xl sm:text-2xl font-bold text-primary">{t("title")}</h2>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center w-full sm:w-auto">
                        ...
                        className="px-3 py-2 rounded-md border border-border bg-bg-offset text-text shadow-sm w-full sm:w-56"
                        />

                        <select
                        value={sort}
                        onChange={(e) => setSort(e.target.value)}
                        className="px-3 py-2 rounded-md border border-border surface-offset text-text w-full sm:w-auto"
                        >
                ...
                {/* Tags + Tech */}
                <section className="mb-6 flex flex-col sm:flex-row flex-wrap gap-3 items-start sm:items-center">
                {/* Tags */}
                ...
                {filtered.map((post) => (
                    <article
                    key={post.id}
                    className="bg-bg-offset p-5 rounded-lg shadow-sm border border-border"
                    >
                    <div className="flex items-start justify-between">
                        <div>
                        <h2 className="text-lg sm:text-xl font-semibold mb-1 text-text">
                            {post.title}
                        </h2>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                    ...
                    />
                    <div className="relative max-w-lg sm:max-w-2xl md:max-w-3xl w-full surface-offset p-6 rounded-lg shadow-lg z-10 overflow-auto max-h-[90vh] border border-border">
